from image_labelling_tool import handler
...
class LabellingToolViewWithLocking (LabellingToolView):
    ...
    def update_labels(self, request: HttpRequest, image_id_str: str, labels_json: Any, completed_tasks: Container[str],
        ...
        image = self.get_image(image_id_str)
        ...
        scheme = models.LabelClass.objects.all()
        class_labbelling_scheme = {class_.name: class_.id for class_ in scheme}

        db_handler = handler.DatabaseHandler([image])
        ...
        fields = ['name', 'model', 'annotation_type', 'photo']

        request_json = {field: annotation_json[field] for field in fields}
